...
def imdb_load(type):
    ...
    labels = labels[index]
    file_lists = file_lists[index]
    # 租个处理文件
    sentences = []
    for file in file_lists:
        # print(file)
        sentences.append(prepross(file))
...
def train():
    ...
    # 通过layer名字获取layer的信息
    print(model.get_layer("embedding").input_shape)
    # 基于此向量的堆叠方式做卷积
    ...
    # 二分类问题,使用sigmoid函数做分类器
    model.add(keras.layers.Activation(activation=keras.activations.sigmoid, name="sigmoid"))
    # 打印模型各层layer信息
    model.summary()
    ...
    history = LossHistory()
    train_history = models.fit(
                        )
    '''
    # fit返回log中,有epochs组数据,即只保存每个epoch的最后一次的loss等值
